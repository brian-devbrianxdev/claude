...
| **advisor** (scarce, above opus) | `fable` | Manual-only: provides high-risk **recommendation** (not final authority). The orchestrator verifies advisor findings and owns the user-facing Go/No-Go verdict. Eligible only after Opus when: ≥2 viable approaches remain open, two attempts failed, rollback cost is high, or final sign-off on genuinely high-risk work. Never auto-spawned — see [`../../MODEL_ROUTING.md`](../../MODEL_ROUTING.md) |
