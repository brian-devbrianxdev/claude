...
| **highest available** (session default — Fable/Opus, whatever the user runs) | *(omit `model` — inherit)* | Orchestration only: planning and decomposing large work, coordinating multiple agents, synthesizing several workers' results. Does **not** own Go/No-Go verdicts on high-risk changes — that is the advisor's role. |
| **advisor** (scarce, above opus) | `fable` | Manual-only: provides high-risk recommendation to the orchestrator. Go/No-Go authority for architecture decisions, root cause unclear after real investigation, failed fix attempts, cross-repo/contract/security/consistency risk, or final sign-off on a high-risk change. The orchestrator verifies advisor findings and issues the user-facing verdict. Never auto-spawned — see [`../../MODEL_ROUTING.md`](../../MODEL_ROUTING.md) |
